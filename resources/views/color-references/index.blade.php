@extends('layouts.app')
@section('title', $label)
@section('meta_description', 'Browse ' . $label . ' — curated color resources for modern designers.')

@section('content')
<div class="vk-container" style="padding-top:64px;padding-bottom:64px;">
    <div style="margin-bottom:48px;">
        <p style="font-size:11px;font-weight:600;color:#787583;text-transform:uppercase;letter-spacing:0.08em;margin:0 0 10px;">Color</p>
        <h1 class="page-title" style="margin:0 0 12px;">{{ $label }}</h1>
        <p style="font-size:16px;color:#454F5E;margin:0;">Curated color resources with copy-ready CSS.</p>
    </div>
    ...
    <div style="display:flex;flex-wrap:wrap;gap:8px;margin-bottom:48px;align-items:center;">
        ...
        <a href="{{ url()->current() }}" class="filter-pill {{ !request('style') ? 'active' : '' }}">All</a>
        @foreach($styles as $s)
        <a href="{{ url()->current() }}?style={{ $s }}" class="filter-pill {{ request('style') === $s ? 'active' : '' }}">{{ ucfirst($s) }}</a>
        @endforeach
    </div>
    @else
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        @foreach($references as $reference)
            @include('partials.color-reference-card', ['reference' => $reference, 'type' => $type])
        @endforeach
    </div>
    <div style="margin-top:40px;">{{ $references->links() }}</div>
    @endif
</div>
@endsection
